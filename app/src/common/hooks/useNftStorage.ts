...
export const useNftStorage = () => {
  ...
  const uploadNftData = async (collectionName: string, file: File) => {
    ...
    console.log(
      `Upload ${imageFile.name} to NFT Storage took ${timerEnd - timerInit}ms`
    );
    setNftData((prev) => ({ ...prev, [imageFileText]: metadata }));
    return metadata;
  };
